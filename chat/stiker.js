...
exports.stiker = async (rahman, message) => {
  ...
  if (isMedia && type === 'image') {
    ...
    await rahman.sendImageAsSticker(from, imageBase64, configStiker)
    ...
    const url = args[1];
    if (url.match(isUrl)) {
      await rahman.sendStickerfromUrl(from, url, { method: 'get' }).catch(err => console.log('Caught exception: ', err))
    } else {
